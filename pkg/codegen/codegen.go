...
func (g *genVisitor) renderUnmarshal(structName, graphqlObjectName string, ref ast.Node) {
	...
	case ast.NodeKindDirectiveDefinition:
		g.file.Func().Params(jen.Id(strings.ToLower(structName)[0:1]).Id("*").Id(structName)).
			Id("Unmarshal").Params(
			jen.Id("doc").Id("*").Qual("github.com/daniil-kzn/graphql-go-tools/pkg/ast", "Document"),
			jen.Id("ref").Int()).
...
func (g *genVisitor) LeaveEnumTypeDefinition(ref int) {
	...
	g.file.Func().Params(jen.Id(shortHandle).Id("*").Id(name)).Id("Unmarshal").Params(jen.Id("doc").Id("*").Qual("github.com/daniil-kzn/graphql-go-tools/pkg/ast", "Document"), jen.Id("ref").Int()).Block(
